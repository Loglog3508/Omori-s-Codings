...
(function(global){
  ...
  const LS_PREFIX = 'ocean_platform_cache_v1_';
  ...
  function fallbackPlasticWaste(){
    return [
      { country: 'China', year: 2019, mismanaged: 8800000, total: 60000000, per_capita: 42 },
      { country: 'United States', year: 2019, mismanaged: 730000, total: 42000000, per_capita: 128 },
      { country: 'Indonesia', year: 2019, mismanaged: 3400000, total: 8700000, per_capita: 32 },
      { country: 'India', year: 2019, mismanaged: 3300000, total: 26000000, per_capita: 19 },
      { country: 'Brazil', year: 2019, mismanaged: 500000, total: 11000000, per_capita: 52 },
      { country: 'Russia', year: 2019, mismanaged: 1500000, total: 8000000, per_capita: 55 },
      { country: 'Turkey', year: 2019, mismanaged: 1100000, total: 6100000, per_capita: 73 },
      { country: 'Philippines', year: 2019, mismanaged: 2000000, total: 6100000, per_capita: 55 }
    ];
  }
  ...
  async function getPlasticWasteTimeSeries(opts={}){
    return getOrFetch('plastic_waste_timeseries_v1', async ()=>{
      ...
      const values = years.map(y=> +(agg.get(y)/1e6).toFixed(2)); // 转换为百万吨
  ...
  async function getOceanCurrentField(bbox={ latMin:0, latMax:30, lonMin:100, lonMax:140 }){
    return getOrFetch('ocean_currents_oscar_v1', async ()=>{
      const url = 'https://coastwatch.pfeg.noaa.gov/erddap/griddap/oscar_vel1819_0.json?u[0:1:0][0:1:60][0:1:120]&v[0:1:0][0:1:60][0:1:120]';
      const resp = await fetchWithTimeout(url, 12000);
      const json = await resp.json();
      return json; // 暂不过滤。后续可抽取稀疏矢量
