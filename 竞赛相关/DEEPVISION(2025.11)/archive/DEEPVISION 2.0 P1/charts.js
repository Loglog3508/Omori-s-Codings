...
class ChartManager {
  ...
  showLoadingOverlay(canvas,text='加载中...'){
    this.hideLoadingOverlay(canvas); // 防止重复叠加导致旧 overlay 无法移除
  ...
  async createOceanCurrentChart(updateOnly=false){
    // 1. 获取当前 DOM 中的 canvas 元素
    let speedCtx=document.getElementById('ocean-river-chart');
    let overviewCtx=document.getElementById('ocean-overview-chart');
    if(!speedCtx && !overviewCtx) return;
    if(!window.dataService){ console.warn('[OceanCurrent] dataService 缺失'); return; }
    
    // 2. 检查并清理旧实例 (如果 Canvas 元素已变更)
    if(this.charts.oceanRiver && this.charts.oceanRiver.canvas !== speedCtx){
        this.charts.oceanRiver.destroy();
        this.charts.oceanRiver = null;
    }
    if(this.charts.oceanOverview && this.charts.oceanOverview.canvas !== overviewCtx){
        this.charts.oceanOverview.destroy();
        this.charts.oceanOverview = null;
    }
    ...
    try{
      ...
      // 3. 再次获取 DOM 元素，防止 await 期间 DOM 重建导致引用失效
      speedCtx=document.getElementById('ocean-river-chart');
      overviewCtx=document.getElementById('ocean-overview-chart');
      
      // 移除 loading (确保移除的是当前 DOM 上的 overlay)
      if(speedCtx) this.hideLoadingOverlay(speedCtx);
      ...
      // 检查 overviewCtx 是否可见 (offsetParent不为null)，避免在隐藏 canvas 上绘图报错
      if(overviewCtx && overviewCtx.offsetParent !== null){
        const metrics=[
        ...
        if(this.charts.oceanOverview){ this.charts.oceanOverview.data.labels=L; this.charts.oceanOverview.data.datasets[0].data=V; this.charts.oceanOverview.update(); }
            ...
            this.charts.oceanOverview=new Chart(overviewCtx,{
                type:'bar',
                data:{labels:L,datasets:[{label:'洋流指标',data:V,backgroundColor:c2,borderColor:solid2,borderWidth:2,borderRadius:8,barThickness:32}]},
                options:{
                    indexAxis:'y',
                    responsive:true,
                    maintainAspectRatio:false,
                    interaction: { mode: 'y', intersect: false }, // 优化交互：整行触发
                    plugins:{
                        ...
                        tooltip:{
                            ...
                            padding: 10,
                            callbacks:{
                                label:c=>`${c.dataset.label || ''}: ${c.raw}${U[c.dataIndex]}` // 修复 [object Object]
                            }
                        }
                    },
                    scales:{
                        x:{ticks:{color:'#a8dadc'},grid:{color:'rgba(168,218,220,0.12)'}},
                        y:{ticks:{color:'#a8dadc', font:{size:11}},grid:{display:false}}
                    }
                }
            }); 
            this.appendDataNote(overviewCtx,'来源: OSCAR 洋流 (统计样本)'); 
        }
      }
    }catch(err){ 
        console.warn('[OceanCurrent] 获取失败',err); 
        if(speedCtx) this.hideLoadingOverlay(speedCtx);
    }
    finally{ 
        // 确保在任何情况下都移除 loading，并强制重绘一次以解决布局挤压
        if(speedCtx) {
            this.hideLoadingOverlay(speedCtx);
            // 延迟 resize 解决 flex 布局挤压导致的 tooltip 错位
            setTimeout(()=>{ 
                this.charts.oceanRiver?.resize(); 
                this.charts.oceanOverview?.resize();
            }, 50);
        }
    }
  }
