...
class TwoLayerNet:
    def __init__(self, input_size, hidden_size, output_size, weight_init_std=0.01, rng=None):
        random = rng if rng is not None else np.random.default_rng()
        self.params = {
            'W1': weight_init_std * random.standard_normal((input_size, hidden_size)),
            'b1': np.zeros(hidden_size),
            'W2': weight_init_std * random.standard_normal((hidden_size, output_size)),
